...
pairs = np.array([(l,h) for l in lower for h in higher])
...
def cost(params):
    ...
    try:
        #a.fit()
        return metrics.roc_auc_score(a.manual_labels,a.auto_labels)
    except:
...
for file in tqdm(list_file_to_read,desc='file loop'):
    sub = file.split('_')[0]
    if int(sub[3:]) >= 11:
    ...
    if len(annotation_file) != 0:
        ...
        for p in tqdm(pairs,desc='thresholds'):
            try:
                print(sub,day,p)
                ...
                temp.to_csv(saving_dir+'temp_result3.csv',index=False)
            except:
                print(sub,day,p,print(Counter(a.auto_label)))
            
    else:
        print(sub,day,'no annotation')
result = pd.DataFrame(result)
result.to_csv('optimized3.csv',index=False)
