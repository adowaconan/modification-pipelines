...
for idx in subjectList: # manually change the range, the second number is the position after the last stop
    ...
    for file_to_read in list_file_to_read:    
        ...
        DT_C=np.array(DT_C)
        ASI = np.array(ASI)
        #change_alpha = np.concatenate(([0],np.diff(np.array(psd_alpha).mean(1))))
        range_alpha = np.array(psd_alpha).mean(1).max() - np.array(psd_alpha).mean(1).min()
        #change_beta = np.concatenate(([0],np.diff(np.array(psd_beta).mean(1))))
        range_beta = np.array(psd_beta).mean(1).max() - np.array(psd_beta).mean(1).min()
        power_fast_spindle = np.array(fast_spindle)
        #chagne_fast_spindle
        range_fast_spindle = np.array(fast_spindle).mean(1).max() - np.array(fast_spindle).mean(1).min()
        My_ASI = (np.log2(np.array(psd_alpha).mean(1)/range_alpha) + np.log2(np.array(psd_beta).mean(1)/range_beta )) / np.log2(power_fast_spindle.mean(1)/range_fast_spindle)
